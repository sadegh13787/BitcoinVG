...
// Copyright (c) 2020 The BitcoinVG Core developers
...
{
    // Force block reward to zero when 21Million total reached
    if (nHeight >= 2000000)
        return 0;

    // Premine 4.76% to help protect against 51% attacks and support dev activities
    CAmount nSubsidy = [&]() {
        if ( 1 == nHeight )
        {
            return (1000000 * COIN);
        ...
        {
            // Block reward always 10 BVG until 21Million
            return (10 * COIN);
        }
    }();
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.GetHash() != chainparams.GetConsensus().hashGenesisBlock)
    {
        // 51% attack protection - Proof of Transaction
        double nValueOutAdj = nValueOut/100000000.0;
        nValueOutAdj /= 100.0;
        // 8bits worth of coin per block (25600 BVG) is enough to follow the money
        if (nValueOutAdj > 255)
        {
            nValueOutAdj = 255;
        }
        nValueOutAdj = static_cast<uint32_t>(nValueOutAdj);

        // Make sure the hash holds the Proof of Transaction vouts that resides in the upper 16 bits.
        // More vouts equate to more work, the chain follows the money.
        uint32_t hash_last32 = get_32bit_word_from_uint256(block.GetHash());
        uint32_t vouts_from_hash = hash_last32 & 0x000000FF;

        if (nValueOutAdj != vouts_from_hash)
        {
            //error("ConnectBlock1(): Bad vouts in hash: %d vs %d", nValueOutAdj, vouts_from_hash);
            return false;
        }
    }
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckPOW)
    {
        if (block.GetHash() != consensusParams.hashGenesisBlock)
        {
            ...
            if (nValueOutAdj != vouts_from_hash)
            {
                //error("ConnectBlock2(): Bad vouts in hash: %d vs %d", nValueOutAdj, vouts_from_hash);
                return false;
            }
        }
    }
